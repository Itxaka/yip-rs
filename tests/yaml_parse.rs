...
/// Go It #6 (line 90): "Reads cloudconfig to boot stage". Cloud-init keys
/// route into boot / initramfs / network stages; assertions match Go.
#[test]
#[ignore = "requires #cloud-config → yip transform (FromFile/Load); not yet implemented in yip-rs"]
fn go_it_reads_cloudconfig_to_boot_stage() {
    let y = indoc! {r#"
        #cloud-config
        growpart:
         devices: ['/']
    ...
    assert_eq!(boot0.users["bar"].uid, "1002");
    assert_eq!(boot0.users["bar"].password_hash, "foo");
    let mut expected_ssh: HashMap<String, Vec<String>> = HashMap::new();
    expected_ssh.insert("bar".into(), vec!["faaapploo".into(), "asdd".into()]);
    assert_eq!(boot0.ssh_keys, expected_ssh);
    assert_eq!(boot0.files[0].path, "/foo/bar");
    assert_eq!(boot0.files[0].permissions, 0o644u32);
    assert_eq!(boot0.hostname, "");
    assert_eq!(cfg.stages["initramfs"][0].hostname, "bar");
    assert_eq!(boot0.commands, vec!["foo".to_string()]);
    assert_eq!(cfg.stages["test"][0].environment.get("foo").unwrap(), "bar");
    assert!(boot0.users["bar"].lock_passwd);
    let boot1 = &cfg.stages["boot"][1];
    assert_eq!(boot1.layout.expand.as_ref().unwrap().size, 0u64);
    assert_eq!(boot1.layout.device.as_ref().unwrap().path, "/");
}

/// Go It #7 (line 133): "Reads sshkeys to network stage if they require
/// network". A `gitlab:test` key must land in the `network` stage; plain
/// keys stay in `boot`.
#[test]
#[ignore = "requires #cloud-config → yip transform (FromFile/Load); not yet implemented in yip-rs"]
fn go_it_reads_sshkeys_to_network_stage_if_they_require_network() {
    let y = indoc! {r#"
        #cloud-config
        growpart:
         devices: ['/']
    ...
    assert_eq!(boot0.users["bar"].uid, "1002");
    assert_eq!(boot0.users["bar"].password_hash, "foo");
    assert_eq!(boot0.ssh_keys.len(), 1);
    let mut expected_boot_ssh: HashMap<String, Vec<String>> = HashMap::new();
    expected_boot_ssh.insert("bar".into(), vec!["asdd".into()]);
    assert_eq!(boot0.ssh_keys, expected_boot_ssh);
    assert_eq!(boot0.files[0].path, "/foo/bar");
    assert_eq!(boot0.files[0].permissions, 0o644u32);
    assert_eq!(boot0.hostname, "");
    assert_eq!(cfg.stages["initramfs"][0].hostname, "bar");
    ...
    assert_eq!(cfg.stages["network"][0].ssh_keys.len(), 1);
    let mut expected_net_ssh: HashMap<String, Vec<String>> = HashMap::new();
    expected_net_ssh.insert("bar".into(), vec!["gitlab:test".into()]);
    assert_eq!(cfg.stages["network"][0].ssh_keys, expected_net_ssh);
}

/// Go It #8 (line 181): "Reads cloudconfig with a jinja header". The
/// `## template: jinja` shebang must be stripped before the
/// `#cloud-config` detection.
#[test]
#[ignore = "requires #cloud-config + jinja header handling; not yet implemented in yip-rs"]
fn go_it_reads_cloudconfig_with_a_jinja_header() {
    let y = indoc! {r#"
        ## template: jinja
        #cloud-config
        users:
    ...
    let cfg = Config::load(y.as_bytes()).unwrap();
    assert_eq!(cfg.stages.len(), 3);
    assert_eq!(cfg.stages["boot"][0].users["bar"].name, "bar");
}
...
#[test]
#[ignore = "serde rejects null for sequence/map fields by default; widening to \
            'null-tolerates-default' for every field would require custom \
            deserializers across the whole schema. Documented divergence."]
fn fuzz_null_for_every_stage_field() {
    // YAML null on every field — should also produce a default Stage.
    let y = indoc! {r#"
        commands: null
        files: null
        downloads: null
        directories: null
        if: null
        ensure_entities: null
        delete_entities: null
        dns: null
        hostname: null
        name: null
        sysctl: null
        ...
        datasource: null
        layout: null
        systemd_firstboot: null
        timesyncd: null
        git: null
        only_os: null
        only_os_version: null
        only_arch: null
        only_service_manager: null
        if_files: null
    "#};
    let s: Stage = serde_yaml::from_str(y).expect("null-everywhere stage should parse");
    assert_eq!(s, Stage::default());
}
